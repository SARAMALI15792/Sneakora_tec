...
export default function SignUpPage() {
  ...
  function goToSignIn() {
    navigateWithTransition(router, "/sign-in");
  }
    ...
    const { error: err } = await authClient.signUp.email(
      { name, email, password, callbackURL: "/" },
      {
        onSuccess: () => {
          ...
          setTimeout(() => navigateWithTransition(router, "/"), 600);
        },
        onError: (ctx) => {
    ...
    <div className="flex min-h-[calc(100dvh-4rem)] items-center justify-center px-6 py-24">
      <Card className="w-full max-w-sm">
        <CardHeader className="text-center">
          ...
          <CardDescription>Join Sneakora and discover premium sneakers</CardDescription>
        </CardHeader>
